 ...
 * world, so the response shape that surfaces it must also be a string.
 * The `auditId` field is opaque to clients — they only carry it as a
 * correlation handle — so this is not a wire-compat change.
 *
 * Read-only entry point over the `AuditRepository`. The command is
 * the only place that knows the "read audit history" contract:
 * validate → paginate within the existing port's surface → shape
 * the response.
 *
 ...
 *     is real Mongo `find().sort({occurredAt: -1}).limit(N)`; the
 *     test adapter is an in-memory fake.
 ...
 *     Express request log), not the application command.
 *
 * **Pagination.** The current `AuditRepository.listByIdentity` port
 * exposes only `limit` (no `offset`, no cursor). So the command
 * offers a *windowed* read: a single page of up to `limit` records,
 * ordered by the adapter's own canonical ordering (most recent
 * first; the production adapter enforces this in
 * `.sort({occurredAt: -1})`). Future sessions that introduce
 * offset/cursor-based pagination on the audit port will not need to
 * change this command's surface.
 *
...
/**
 * Caller context. Mirrors `TokenizeCallerContext` so the audit
 * chain downstream sees a consistent actor triple (who, why, from
 * where) regardless of which command wrote it. The context here is
 * *not* persisted as a new audit row (read = read; see the file-
 * level comment), but it is the right shape for a future route
 * handler that wants to log the read alongside the call site.
 *
 * `requestId` is opaque — typically the inbound HTTP `X-Request-Id`
 * or an upstream correlation id. `actorRole` is one of the
 * well-known RBAC tags; kept as a string union so future roles
 ...
 *
 * NOTE on the auditId type: the original Postgres-backed command
 * used `number` (BIGSERIAL). The new Mongo-backed command uses
 * `string` (stringified ObjectId). The new shape is the one this
 * ported command returns; clients that correlated audit ids to
 * Postgres BIGSERIALs need to update their parsers, but the field
 * is opaque so the migration is mechanical.
 */
...
export function makeReadAuditHistory(deps: ReadAuditHistoryDeps) {
  return async function readAuditHistory(
    ...
    // -----------------------------------------------------------------
    // 2. Resolve `limit`. Default when absent; validate when
    //    present; clamp to MAX_LIMIT. We do NOT reject an
    //    over-large limit — silent clamping is friendlier to API
    //    clients and the `limit` field in the response echoes the
    //    effective value.
    // -----------------------------------------------------------------
    ...
    //    `AuditRecord[]` ordered by the adapter's canonical
    //    ordering (most recent first; the production Mongo
    //    adapter enforces this with `.sort({occurredAt: -1})`).
    //    The command does not sort, deduplicate, or enrich the
    //    rows.
    // -----------------------------------------------------------------
 ...
 * `occurredAt` is serialized to ISO-8601 so the caller never has
 * to deal with `Date` instances. Optional fields are normalized to
