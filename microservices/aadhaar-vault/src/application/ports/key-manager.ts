 ...
 * The port is intentionally narrow — three methods only — so adapters
 * can be swapped without leaking provider semantics into the call
 * sites.
 *
...
/**
 * Opaque context bytes that bind a DEK to its use site (e.g. an
 * identity_id, a per-record salt). Different `context` inputs to the
 * same master key MUST produce different wrapped DEKs even when the
 * plaintext DEK is identical.
 */
